<?php
	...
	// for consensus two, need more info - num domains per method - whether method fragments, and the sec structure of method - again use arrays
	$methDomains = array();    // method will be key, num of domains value
	$methFragments = array();  // will show whether method makes fragmented domains - key method, value 1 if none, integer value higher if so
	$numFragmentedMeths = 0;   // used to keep track of number of methods that fragment - easier to access for weight generation

	// each method also has a weighting towards consensus, the initial weighting it has is set here, and altered later
	$methWeight = array();      //key method, value weight
	$methWeight["pdp"] = 84.4;
	$methWeight["NCBI"] = 81.9;
	$methWeight["dp"] = 78.1;
	$methWeight["DDomain"] = 76.5;
	$methWeight["DHcL"] = 68.3;
	$methWeight["PUU"] = 74;
	$methWeight["Dodis"] = 40;
	...
	*/
	function generateSearchString()
	{
		...
		{
			if ($answer !="true")         // the user has not selected this method
			{
	...
	*/
	...
	/**
	* analyseChain - goes through the chain to determine the secondary structure makeup (all alpha-helix, all beta sheet, mix of both)
	*/

	function analyseChain()
	{
		global $structId,$chainId,$dbname,$con,$selMeths;
		// now query DSSP table to find any beta sheets ("E" in table)
		$betaQur = mysql_query("SELECT secondaryStructure FROM dssp WHERE structId='".$structId."' and chainId='".$chainId."' and secondaryStructure='E'");
		// now query DSSP table to find any helices (G,H,I)
		$helixQur = mysql_query("SELECT secondaryStructure FROM dssp WHERE structId='".$structId."' and chainId='".$chainId."' and secondaryStructure='H'");
		//print mysql_num_rows($betaQur).' '.mysql_num_rows($helixQur).'<BR>';	

		if(mysql_num_rows($betaQur)==0)             // no beta sheets in structure
		{
			if($helixQur!=0)     // error check
			{
				return "helix";
			}
		}
		else if(mysql_num_rows($helixQur)==0)
		{
			//print 'HERE';
			if($betaQur!=0)
			{
		...
		{
			// neither null, so must be a mix of both
			return "both";
		}
	}
	...
	{
		global $structId,$chainId,$dbname,$con,$selMeths,$methDomains,$methFragments,$numFragmentedMeths;
		...
		// add the domain count to the array, used to work out the weighted consensus later
		$methDomains[$method] = $domCount;
		...
		// add the fragment count to the fragment per method array, used to work out the weighted consensus later
		$methFragments[$method] = $fragNum;
		$numFragmentedMeths = $numFragmentedMeths+1;
		//print $method.'<BR>'.$fragNum.'<BR>';
	...
	*/		
	...
	/**
	* generateDomainWeights - goes through the methods and adjusts the contributory weight based on their performance on examination
	* of how many domains each method assigns
	*
	* $methods	ResultSet containing the methods that have made an assignment for this chain & selected by user
	*/
	function generateDomainWeights($numMethods)
	{
		global $finalMap,$weights,$methDomains,$methWeight;
		// now adapt the initial weights following the domain rules set

		// 1. If num domains in PDP and NCBI>=4, downgrade DP by 10%
		if(($methDomains["pdp"]>=4) && ($methDomains["NCBI"]>=4))
		{
			$methWeight["dp"] = ($methWeight["dp"]-10);
		}

		// 2. If PUU has more domains than PDP and NCBI, downgrade PUU by 10%
		if(($methDomains["PUU"]>$methDomains["pdp"]) && ($methDomains["PUU"]>$methDomains["NCBI"]))
		{
			$methWeight["PUU"] = ($methWeight["PUU"]-10);
		}

		// 3. If PDP Num Domains >=5 and NCBI disagrees, downgrade NCBI by 10%
		if(($methDomains["pdp"]>=5) && ($methDomains["pdp"]!=$methDomains["NCBI"]))
		{
			$methWeight["NCBI"] = $methWeight["NCBI"]-10;
		}
	}

	/**
	* generateFragmentWeights - goes through the methods and adjusts the contributory weight based on their performance on fragmentation
	* of domains
	*
	* $methods	ResultSet containing the methods that have made an assignment for this chain & selected by user
	*/
	function generateFragmentWeights($methods)
	{
		mysql_data_seek($methods,0);
		global $finalMap,$weights,$methDomains,$methWeight,$methFragments,$numFragmentedMeths;
		// now adapt the initial weights following the fragment rules set
		
		// 1. 3 or more have 1 fragmented domain, downgrade all methods that do not predict fragments by 10%
		if($numFragmentedMeths>=3)
		{
			// now downgrade the methods that do not fragment
			while($row = mysql_fetch_array($methods))
			{
				$method = $row["method"];
				if($methFragments[$method]==1)    // there are fragments for this method
				{
					$methWeight[$method] = $methWeight[$method]-10;
				}
			}
		}

		// 2. If NCBI and PDP do not have fragments, downgrade all methods with fragments by 10%
		if(($methFragments["NCBI"]==1) && ($methFragments["pdp"]==1))
		{
			mysql_data_seek($methods,0);
			// go through each method locating those that fragment and downgrade by 10%
			while($row = mysql_fetch_array($methods))
			{
				$method = $row["method"];
				if($methFragments[$method]>1)    // there are fragments for this method
	...
	/**
	* generateStructureWeights - goes through the methods and adjusts the contributory weight based on their performance on examination
	* of the chains secondary structure
	*
	* $methods	ResultSet containing the methods that have made an assignment for this chain & selected by user
	*/
	function generateStructureWeights($methods)
	{
		global $finalMap,$weights,$methDomains,$methWeight,$methFragments,$numFragmentedMeths;
		// now adapt the initial weights following the fragment rules set

		// Firstly, analyse the chain to see if all alpha-helix, all beta sheet, or contains both
		$structure = analyseChain();

		//1. the structure is all alpha-helix, then if NCBI and PDP disagree on number of domains, increase PDP by 10%.
		if(strcmp($structure,"helix")==0)
		{
			if($methDomains["pdp"]<>$methDomains["NCBI"])
			{
				$methWeight["pdp"] = $methWeight["pdp"]+10;
			}
		}
		//2. If structure is all beta-sheet, then:
		if(strcmp($structure,"sheet")==0)
		{
			//If NCBI and PDP disagree  (on the number of domains) – increase PDP contribution by 10%
			if($methDomains["pdp"]<>$methDomains["NCBI"])
			{
				$methWeight["pdp"] = $methWeight["pdp"]+10;
			}
			//If NCBI and PDP agree, increase contribution of both by 10%
			else
			{
				$methWeight["NCBI"] = $methWeight["NCBI"]+10;
				$methWeight["pdp"] = $methWeight["pdp"]+10;
		...
		//3. If the structure is an alpha-beta (both beta-sheets and alpha- helices are present) then if NCBI and PDP agree, reduce all methods that disagree with them by 10%.
		if(strcmp($structure,"both")==0)
		{
			if($methDomains["pdp"]<>$methDomains["NCBI"])
			{
				mysql_data_seek($methods,0);
				// go through each method locating those that disgaree with both and downgrade by 10%
				while($row = mysql_fetch_array($methods))
				{
					$method = $row["method"];
					if($methDomains[$method]!=$methDomains["pdp"])        // no need to compare with NCBI as same as pdp
					{
						$methWeight[$method] = $methWeight[$method]-10;
					}
				}
	...
	/**
	* sumWeights - takes the generated weight of each method and adds them, getting an overall total weight
	*
	* @param $methods	ResultSet containing the methods that have made an assignment for this chain & selected by user
	* @return $total	The total of the weights
	*/
	function sumWeights($methods)
	{
		global $finalMap,$weights,$methDomains,$methWeight,$methFragments,$numFragmentedMeths;
	 ...
	 * determined as the total of the weights assigned to each method in the group divided by the total number of the weights assigned to all 
	 * methods  This is also stored in a map (weights) which is used later to generate a consensus.  Note the key
	 ...
	 * @return null
	 */
	function generateWeights($methods)
	{
		global $finalMap,$weights,$methDomains,$methWeight,$methFragments,$numFragmentedMeths;
		// Now that the weights have all been adjusted, can work out the final weight
		// To do this, the sum of the adjusted weights is necessary
		$totalWeight = sumWeights($methods);
		foreach ($finalMap as $value)
		{
			$groupWeight = 0;
			// $value is an array containing the group
			// now work out the total weight in this group
			foreach($value as $member)
			{
				$groupWeight = $groupWeight + $methWeight[$member];
			}
			$weight = $groupWeight/$totalWeight;
			// add to the weights map - consensus is now found using exactly the same method as previously with consensus 1
			$weights[] = $weight;
		}
	}
		
	/** 
	* generateConsensus - takes the weights assigned to the groups made previously, and attempts to find a consensus domain assignment,
	* which is returned
	*
	* @return consensus	RecordSet containing the assigned consensus
	*/
	function generateConsensus()
	{
		...
		else if($maxRelWeight>=0.4 && $nextHighWeight!=0)     // there are two options and no strong consensus, so display both options
		{
			...
			$groupList = $methsNext;                        // set global variable to point to the best consensus methods
			$altGroupList = $methsHigh;			// global variable with methods for second group of consensus
	...
	/**
	* getConsensus - the main method here - gets the input from the form, the chain length, and attempts to generate a consensus using
	* all the methods described previously.  This is returned so that if a consensus has been found, this can be drawn on the web page
	*/
	function getConsensus()
	{
		global $dbname,$con,$continuousMap,$splitDomMap,$methWeight;
		mysql_select_db($dbname, $con);
		...
		if($length>0)           // a length result was returned
		{
			...
			joinMaps();
			generateDomainWeights($numMethods);
			generateFragmentWeights($methods);
			generateStructureWeights($methods);
			generateWeights($methods);
	...
	/**
	* viewConsensus - test method - used to print the consensus resultset to the screen
	*
	* @param $consensus	ResultSet containing the generated consensus
	*/
	...
	// Now get consensus if the chain exists
	if($chainStart!=null)
	{
		print '
		<table width="95%"  border="0" cellspacing="0" cellpadding="0">
			<tr align="left">
			  ...
			  PrintSideMenuBox("Results",ResultSideMenuItems("Results","Weighted Consensus",$structId,$_GET["chain"],$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"]));
			  print '
			  </td>
			  <td valign="top" align="left">
	        <div id="textheading">Weighted Consensus</div>
		...
		if($consensus!=null)
		{
			...
			print '
				<TABLE WIDTH="95%">
				<TR><TH WIDTH="60%" ALIGN=CENTER>Scores<TH WIDTH="15%"><TH WIDTH="25%" ALIGN=CENTER>Key</TR>
				...
				print '</TD><TD>[40-59]: Weak Consensus</TD></TR>
				...
				// PRINT ADJUSTED WEIGHT PER METHOD
	
				print '<TABLE><TR><TD><b><A HREF="consensus2rules.php?pdbId='.$structId.'&chain='.$_GET["chain"].'&CATH='.$selMeths	["CATH"].'&SCOP='.$selMeths["SCOP"].'&pdp='.$selMeths["pdp"].'&dp='.$selMeths["dp"].'&DHcL='.$selMeths["DHcL"].'&DDomain='.$selMeths["DDomain"].'&PUU='.$selMeths["PUU"].'&NCBI='.$selMeths["NCBI"].'&Dodis='.$selMeths["Dodis"].'">Adjusted Weights for methods</b></TD>';


				//print '<TABLE><TR><TD><b>Adjusted weight for methods:</b></TD> ';
				$i = 0;
				while($i<sizeof($groupList))
				{
					print '<TD>'.$groupList[$i].': '.$methWeight[$groupList[$i]].'</TD></TR><TR><TD></TD>';
					$i=$i+1;
				}
				print '</TR></TABLE></TD><TD></TD><TD></TD></TR>
				<TR><TD>';
				...
				print '</TR></TD><TD></TD><TD></TD></TR><TR><TD>';

				// PRINT ADJUSTED WEIGHT PER METHOD FOR SECOND CONSENSUS OPTION IF EXISTS
				if(($maxRelWeight-$nextRelWeight)<=0.1)
				{
					// Note this label acts as a link to the description of consensus calculation (explains long link)
					//print '<TABLE><TR><TD><b>Adjusted weights for methods:</b></TD> ';
					print '<TABLE><TR><TD><b><A HREF="consensus2rules.php?pdbId='.$structId.'&chain='.$_GET["chain"].'&CATH='.$selMeths["CATH"].'&SCOP='.$selMeths["SCOP"].'&pdp='.$selMeths["pdp"].'&dp='.$selMeths["dp"].'&DHcL='.$selMeths["DHcL"].'&DDomain='.$selMeths["DDomain"].'&PUU='.$selMeths["PUU"].'&NCBI='.$selMeths["NCBI"].'&Dodis='.$selMeths["Dodis"].'">Adjusted Weights for methods</b></TD>';

					$i = 0;
					while($i<sizeof($altGroupList))
					{
						print '<TD>'.$altGroupList[$i].': '.$methWeight[$altGroupList[$i]].'</TD></TR><TR>';
						$i=$i+1;
					}
				}
				print '</TR></TD><TD></TD><TD></TD></TR></TABLE>';
			...
			//if($selMeths[$currentMethod]=="true")
				print'<tr><td colspan=3>  </td></tr><tr><td width="18%" align=center>Consensus '.$consensusCount.'</td>';
			while($row = mysql_fetch_array($consensus))
			{
				$struct = $row['domain'];
				$fragment = $row['fragmentNo'];
				...
				if($selMeths[$method]=="true")         // the user has selected the method
				{
					...
					else           // different method, so currentmethos needs changing & method printing
					{
						...
						$currentDomain = $struct;
						$fragmentPrint = $start.'-'.$end;
						print'<tr><td width="18%" align=center>Consensus'.$consensusCount.'</td>';
					}
				}
		...
		print '<BR><b>Explanation of Weighted Consensus Process</b><BR><BR>

		The above shows the result gained through adopting the Weighted Consensus Approach in order to generate a consensus domain assignment for this chain using the results of each automatic algorithms prediction.  How this compares with CATH and SCOP is also shown.    
		<BR><BR>
		...
		<BR><BR>
		The way this consensus differs from the previous approach is the generation of the score.  Once again, once the methods are in groups, a score is calculated for the group.  In this case, each algorithm has an initial weighting.  The weighting is either increased or reduced based on the methods performance in comparison to the rest of the methods.  The rules for increasing or decreasing the assigned weight can be seen by <A HREF="consensus2rules.php?pdbId='.$structId.'&CATH='.$selMeths["CATH"].'&SCOP='.$selMeths["SCOP"].'&pdp='.$selMeths["pdp"].'&chain='.$chainId.'&dp='.$selMeths["dp"].'&DHcL='.$selMeths["DHcL"].'&DDomain='.$selMeths["DDomain"].'&PUU='.$selMeths["PUU"].'&NCBI='.$selMeths["NCBI"].'&Dodis='.$selMeths["Dodis"].'">clicking here</A>.  Once the weighting for each method has been adjusted, the total of all weights is generated.  The score of each group is then set to be the total of all weights assigned to the methods in that group divided by the total of all the weights
		<BR><BR>   
		Again, the group that has the highest reliability score over 40% is recognised as the consensus.  Should any group have a reliability score within 10% of the highest group, this is also presented as a potential consensus.
		<BR><BR>
		In some cases, no group of methods scores over 40%  In these cases, it is deemed that no consensus domain assignment can be produced for this chain
		<BR><BR>';
