<?php
	...
	print '
		  <table width="95%"  border="0" cellspacing="0" cellpadding="0">
			<tr align="left">
			  ...
			  PrintSideMenuBox("Results",ResultSideMenuItems("Results","Domain Assignment Summary",$structId,$chainId,$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"],$chainsForMenu));
			  PrintSideMenuBox("Chains",ResultSideMenuItems("Chains",$chainId,$structId,$chainId,$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"],$chainsForMenu));
			  print '
			  </td>
	...
	print '<b>Selected Protein:</b> '.$structId. ' <BR><b>Showing Results for Chain:</b> '.$chainId.'<BR>';
	...
	$getChainStart = mysql_query("SELECT MIN(startPDB) FROM automeths2012 WHERE pdbId='".$structId."' AND chain='".$chainId."'");
	...
	if($chainStart!=null)
	{
		...
		$getChainEnd = mysql_query("SELECT MAX(endPDB) FROM automeths2012 WHERE pdbId='".$structId."' AND chain='".$chainId."' ORDER by endPDB DESC");
		...
		$domains = mysql_query("SELECT pdbId,chain,domain,fragmentNo,startPDB,endPDB,method,FIELD(method,'CATH','SCOP','pdp','DHcL','dp','DDomain','NCBI','PUU','Dodis') AS SORTER FROM automeths2012 WHERE pdbId='".$structId."' AND chain='".$chainId."'ORDER BY SORTER ASC,startPDB");
		...
		$domainstoprint = mysql_query("SELECT pdbId,chain,domain,fragmentNo,startPDB,endPDB,method,FIELD(method,'CATH','SCOP','pdp','DHcL','dp','DDomain','NCBI','PUU','Dodis') AS SORTER FROM automeths2012 WHERE pdbId='".$structId."' AND chain='".$chainId."' ORDER BY SORTER ASC,domain,fragmentNo");
