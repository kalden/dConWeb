	...
	print '
	  <table width="95%"  border="0" cellspacing="0" cellpadding="0">
		<tr align="left">
		  ...
		  PrintSideMenuBox("Results",ResultSideMenuItems("Results","Weighted Consensus - Calculation",$structId,$_GET["chain"],$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"]));
		  print '
		  </td>
		  <td valign="top" align="left">
        <div id="textheading">Weighted Consensus - Generation of Weights</div><BR>

	<div id="textinfo">
	<div style="padding-left:20px; padding-top: 8px; padding-bottom: 8px;">
	Initially, each algorithm is given the following weight:
	<TABLE CELLPADDING=4>
	<TR><TD>PDP<TD>84.4%<TD>NCBI<TD>81.9%<TD>DP<TD>78.1%
	<TD>DDomain<TD>76.5%<TD>DHcL<TD>68.3%<TD>PUU<TD>74.0%
	<TD><TD><TD>Dodis<TD>40.0%<TD><TD></TR>
	</TABLE>
	<BR>
	Once all the methods have been grouped, the weights are adjusted based on three sets of rules.  These are:

	<li>
	<ul><b>Number of Predicted Domains</b></ul></li>
	If the number of domains predicted by PDP and NCBI >=4, then the weight assigned to DP is reduced by 10%, as DP typically predicts less domains, especially when the true number of domains is 4 or more.<BR><BR>
	If PUU predicts more domains than PDP and NCBI, downgrade PUU prediction by 10%. As noted in previous studies (Holland et al, (2006), Partitioning protein structures into domains: why is it so difficult?, JMB 361, 562-590), PUU tends to overcut a protein chain<BR><BR>
	If PDP predicts five domains or more, downgrade NCBI by 10%<BR><BR>

	<li>
	<ul><b>Number of Fragments per Domain/Chain</b></ul></li>
	If three or more methods have at least one domain fragmented (may not be the same domain) then the weight of all methods that do not predict fragmented domains is reduced by 10%<BR><BR>
	If NCBI and PDP have no fragmented domains, then the weight of all methods that predict fragmented domains is reduced by 10%<BR><BR>

	<li>
	<ul><b>Based on Type of Structure</b></ul></li>
	If the structure is all alpha-helix (in the DSSP structure definition), and NCBI and PDP disagree on the number of domains in the chain, the weight of PDP is increased by 10%<BR><BR>
	If the structure is all beta-sheet, and NCBI and PDP disagree on the number of domains, the weight of PDP is increased by 10%<BR><BR>
	If the structure is all beta-sheet and NCBI and PDP agree, the weight of both methods is increased by 10%<BR><BR>
	If the structure is alpha-beta, and NCBI and PDP agree, the weights of all methods that disagree with PDP and NCBI are reduced by 10%<BR><BR>
