<?php
	...
	if(mysql_num_rows($chains))
	{
		...
		print '
		  <table width="95%"  border="0" cellspacing="0" cellpadding="0">
			<tr align="left">
			  ...
			  PrintSideMenuBox("Results",ResultSideMenuItems("Results","Simple Consensus",$structId,$first_chain,$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"],$chainsForMenu));
			 PrintSideMenuBox("Chains",ResultSideMenuItems("Consensus Chains",$first_chain,$structId,$first_chain,$selMeths["CATH"],$selMeths["SCOP"],$selMeths["DDomain"],$selMeths["Dodis"],$selMeths["PUU"],$selMeths["DHcL"],$selMeths["dp"],$selMeths["pdp"],$selMeths["NCBI"],$chainsForMenu));
			  print '
			  </td>
		...
		print '<b>Selected Protein:</b> '.$_GET["pdbId"]. ' <b>Chain:</b> '.$first_chain.'<BR><BR>';
		print '<b>Result:</b><BR>';
		
		// GET THE CONSENSUS
		$consensus = mysql_query("SELECT * FROM ".$simpleConsensusTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."' ORDER BY consensusNumber,domainStart ASC");	
		...
		if($consensus!=null)
		{
			...
			$getChainStart = mysql_query("SELECT MIN(startPDB) FROM ".$assignmentsTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."'");
			...
			$getChainEnd = mysql_query("SELECT MAX(endPDB) FROM ".$assignmentsTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."'");
			...
			$cathscop = mysql_query("SELECT * FROM ".$assignmentsTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."' and (method='SCOP' or method='CATH') ORDER BY method,start");
			...
			$consensusNumCheck = mysql_query("SELECT MAX(consensusNumber) FROM ".$simpleConsensusTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."'");
			...
			$consensusStats = mysql_query("SELECT * FROM ".$consensusStatsTable." WHERE pdbId='".$structId."' AND chain='".$first_chain."' ORDER BY consensusNumber ASC");
